...
export function PrayerTimesScreen({
  ...
  const { isPlaying, isLoading, currentUrl, togglePreview, stopPreview } = useAzanPreview();

  // Stop preview if sound disabled or component unmounts
  useEffect(() => {
    if (!azanSound) {
      stopPreview();
    }
  }, [azanSound, stopPreview]);
  ...
  const currentAzanSound = customAzanUrl 
    ? (AZAN_SOUNDS.find(s => s.url === customAzanUrl)?.name || 'Custom')
    : AZAN_SOUNDS[0].name;
  ...
  return (
    <div className="animate-screen-in flex flex-col h-full px-4 md:px-8 pt-4 overflow-y-auto hide-scroll pb-8 landscape:pb-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 md:gap-4">
          ...
          <div className="liquid-glass w-full p-4 flex flex-col justify-start items-stretch">
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <ToggleSwitch
                ...
                iconOn={<MapPin className="w-3.5 h-3.5" />}
                iconOff={<MapPin className="w-3.5 h-3.5" />}
              />
            </div>

            <button
              onClick={() => setShowCountrySelector(!showCountrySelector)}
              className="w-full flex items-center justify-between p-2 rounded-lg bg-background/30 mt-2 hover:bg-background/50 transition-colors"
            >
          ...
          <div className="liquid-glass w-full p-4 flex flex-col justify-start items-stretch">
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                disabled={isLoading && currentUrl === getCurrentAzanUrl()}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all ${
                  isPlaying && currentUrl === getCurrentAzanUrl()
                    ? 'bg-accent text-accent-foreground shadow-sm shadow-accent/20' 
                    : 'bg-muted/30 text-muted-foreground hover:bg-muted/50'
                }`}
              >
                {isLoading && currentUrl === getCurrentAzanUrl() ? (
                  <>
            ...
            {azanSound && (
              <>
                <button
                  ...
                  className="w-full flex items-center justify-between p-2 rounded-lg bg-background/30 hover:bg-background/50 transition-colors"
                >
                  <div className="flex items-center gap-2">
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            disabled={isLoading && currentUrl === sound.url}
                          ...
                          >
                            {isLoading && currentUrl === sound.url ? (
                              <Loader2 className="w-3.5 h-3.5 animate-spin" />
                            ) : isPlaying && currentUrl === sound.url ? (
                    ...
                    {showCustomUrlInput && (
                      <div className="flex gap-2 mt-2">
                        <input
                          ...
                          className="flex-1 px-2 py-1 text-xs rounded-lg bg-background/50 text-foreground border border-border/50 backdrop-blur-sm"
                        />
                        <button
        ...
        {/* Right Column: Next Prayer & Prayer Times List */}
        <div className="space-y-4">
          {/* Dates */}
          {date && (
            <div className="text-center md:text-left md:px-2">
              <p className="text-base font-semibold text-foreground">{date}</p>
              {hijriDate && <p className="text-sm text-accent font-medium">{hijriDate}</p>}
            </div>
          )}

          {/* Next Prayer Highlight */}
          {nextPrayer && timeToNext && !loading && (
            <div className="liquid-glass w-full p-4 flex flex-col justify-start items-stretch bg-accent/10 border border-accent/20">
          ...
          {error && !loading && (
            <div className="liquid-glass w-full p-4 flex flex-col items-center justify-center text-center">
              ...
              <button
                onClick={forceRefresh}
                className="mt-2 text-accent text-sm font-semibold hover:brightness-110"
              >
                Try Again
              </button>
            </div>
          ...
          {times && !loading && (
            <div className="space-y-2">
              {Object.entries(times).map(([prayer, time]) => (
                <div
                  key={prayer}
                  className={`liquid-glass w-full p-3.5 flex items-center justify-between transition-all duration-300 ${
                    nextPrayer === prayer
                      ? 'ring-2 ring-accent/60 bg-accent/10 scale-[1.01]'
                      : ''
                  }`}
                >
                  ...
                  <span className={`text-base font-semibold ${nextPrayer === prayer ? 'text-accent font-bold' : 'text-foreground'}`}>
                    {formatTime(time)}
                  </span>
                </div>
              ))}
